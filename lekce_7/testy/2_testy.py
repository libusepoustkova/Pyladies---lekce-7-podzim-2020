...
def test_mladez_za_polovinu():
    assert spocitej_slevu_jizdneho(30.0, 13) == 15.0
...
def test_deti_zdarma():
    assert spocitej_slevu_jizdneho(30.0, 10) == 0

#Pridej test: Zaporny vek skonci vyjimkou ValueError
def test_zaporny():
    with pytest.raises(ValueError):
        spocitej_slevu_jizdneho(30.0, -6)

#Pridej test: Vek 18 se pocita stale jako Mladez (Jezdi za polovinu)
def test_stale_mladez_za_polovinu():
...
def test_zaporna_zakladni_cena():
    with pytest.raises(ValueError):
        spocitej_slevu_jizdneho(-30.0, 6)

# Pridej podminku, ze od 80 let vcetne je jizda zdarma a pridej test, ktery to overi
def test_80tnici_zdarma():
    assert spocitej_slevu_jizdneho(30.0, 80) == 0
